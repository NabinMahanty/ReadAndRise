...
// Verify ownership
$stmt = $pdo->prepare("SELECT id, title, attachment_path FROM notes WHERE id = ? AND user_id = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  $deleteStmt = $pdo->prepare("DELETE FROM notes WHERE id = ? AND user_id = ?");
  if ($deleteStmt->execute([$note_id, $_SESSION['user_id']])) {
    $_SESSION['success'] = "Study material deleted successfully.";
  } else {
    $_SESSION['error'] = "Failed to delete study material.";
  }
...
<div class="card" style="max-width: 600px; margin: 2rem auto;">
  <h2 style="color: #dc2626;">🗑️ Delete Study Material</h2>
  ...
  <div style="background: #fef3c7; padding: 1rem; border-radius: 8px; border-left: 4px solid #f59e0b; margin: 1.5rem 0;">
    ...
    <p style="color: #78350f; margin-top: 0.5rem; margin-bottom: 0;">
      This action cannot be undone. The material and any attached files will be permanently deleted.
    </p>
  </div>

  <div style="background: #f3f4f6; padding: 1rem; border-radius: 8px; margin: 1rem 0;">
    <strong style="color: #1f2937;">Material to be deleted:</strong>
