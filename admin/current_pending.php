  ...
  <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 1rem;">
    <div>
      ...
      <p style="color: #cbd5e1; font-size: 1rem;">
        Review and moderate submitted current affairs articles
      </p>
    </div>
...
<?php if (empty($pending)): ?>
  <div class="card" style="background: linear-gradient(135deg, #1e293b 0%, #334155 100%); border: 1px solid rgba(148, 163, 184, 0.2); padding: 3rem; text-align: center;">
    ...
    <p style="color: #cbd5e1; font-size: 1.1rem;">No pending current affairs posts. Great job! 🎉</p>
  </div>
<?php else: ?>
  <div style="background: linear-gradient(135deg, #1e293b 0%, #334155 100%); padding: 1.5rem; border-radius: 12px; margin-bottom: 1.5rem; border: 1px solid rgba(245, 158, 11, 0.3);">
    <p style="color: #cbd5e1; font-size: 0.95rem; margin: 0;">
      📊 <strong style="color: #fbbf24;"><?php echo count($pending); ?></strong> current affairs article<?php echo count($pending) != 1 ? 's' : ''; ?> awaiting review.
    </p>
  </div>
      ...
      <div class="card" style="background: linear-gradient(135deg, #1e293b 0%, #334155 100%); border: 1px solid rgba(148, 163, 184, 0.2); padding: 1.75rem;">
        <div style="display: flex; justify-content: space-between; align-items: start; gap: 1rem; margin-bottom: 1rem;">
          <div style="flex: 1;">
            ...
            <?php if (!empty($item['summary'])): ?>
              <p style="color: #cbd5e1; margin-bottom: 0.75rem; line-height: 1.6; font-size: 0.95rem;">
                <?php echo nl2br(htmlspecialchars($item['summary'])); ?>
              </p>
            <?php endif; ?>
            <div style="color: #94a3b8; font-size: 0.875rem;">
              <strong style="color: #60a5fa;">✍️ Author:</strong> <?php echo htmlspecialchars($item['author'] ?? 'Admin'); ?>
              <span style="margin: 0 0.5rem;">|</span>
              <strong style="color: #60a5fa;">📅 Submitted:</strong> <?php echo date('d M Y, H:i', strtotime($item['created_at'])); ?>
            </div>
          </div>
        ...
        <?php if (!empty($item['image_path'])): ?>
          <div style="margin-bottom: 1rem;">
            <img src="/ReadAndRise/uploads/current/<?php echo htmlspecialchars($item['image_path']); ?>" alt="" style="max-width: 300px; border-radius: 10px; border: 1px solid rgba(148, 163, 184, 0.2);">
          </div>
        <?php endif; ?>

        <div style="display: flex; gap: 0.75rem; flex-wrap: wrap;">
          <a href="/ReadAndRise/public/current.php?id=<?php echo (int)$item['id']; ?>" target="_blank" style="text-decoration: none;">
          ...
          <a href="current_pending.php?action=approve&id=<?php echo (int)$item['id']; ?>" style="text-decoration: none;">
          ...
          <a href="current_pending.php?action=reject&id=<?php echo (int)$item['id']; ?>" style="text-decoration: none;">
